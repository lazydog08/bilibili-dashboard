...
Mac mini 定时采集默认会在每轮开始前读取本机 Edge 默认用户的
`.bilibili.com` Cookie，并先后核验登录账号和创作中心接口。两项都通过后，
才会以原子方式更新本机运行目录中的 `dashboard.env`；Cookie 值不会写入
日志、Git、NAS 或 Bark。验证失败时仍会更新 B 站公开数据，CTR、平均播放
时长和完播率继续显示旧数据或 `--`，不会冒充本轮实时数据；登录提醒最多
每天一次。浏览器被 B 站主动注销或要求验证码时，需要在这台 Mac mini 的
Edge 完成一次正常登录，后续轮次会自动续用新会话。本项目不会代做或绕过
验证码。

可选配置：

```bash
BILIBILI_BROWSER_COOKIE_REFRESH_ENABLED=1
# 留空即使用 Edge 默认用户；只有使用其他本机用户目录时才填写。
BILIBILI_BROWSER_COOKIE_FILE=
```
...
- 如果启用 `XHS_CREATOR_NOTES_REFRESH_ENABLED=1`，先刷新小红书创作者后台作品明细；`XHS_CREATOR_NOTES_REQUIRED=0` 时采集失败不会阻断主看板更新，`XHS_CREATOR_NOTES_REQUIRED=1` 时才会中止主渲染以防发布旧缓存。
