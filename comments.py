...
def public_comment_item(comment: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "message": sanitize_comment_text(comment.get("message"), 60),
...
def build_comment_context(config: Any = None) -> dict[str, Any]:
    ...
    try:
        ...
        public_items = [public_comment_item(item) for item in raw_items]
        ...
        attention = [
            item
            for item in items
            if item["score"] >= int(getattr(config, "comment_score_push_threshold", 70))
        ...
        latest_items = [
            public_by_id[public_comment_hash(str(item.get("platform") or "bilibili"), str(item.get("comment_id") or ""))]
            for item in latest_raw_items
            if public_comment_hash(str(item.get("platform") or "bilibili"), str(item.get("comment_id") or ""))
            in public_by_id
        ]
        display_items = []
        ...
        return {
            ...
            "items": display_items,
            "message": "完整评论仅保存在本地私有缓存，页面只展示脱敏摘要。",
        }
