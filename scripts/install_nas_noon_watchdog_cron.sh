...
SCHEDULE="${DASHBOARD_NAS_WATCHDOG_CRON_SCHEDULE:-0 12 * * *}"
...
DRY_RUN="${DASHBOARD_NAS_WATCHDOG_CRON_DRY_RUN:-0}"
MARKER_BEGIN="# BEGIN bilibili-dashboard noon watchdog"
MARKER_END="# END bilibili-dashboard noon watchdog"
...
build_watchdog_command() {
  ...
  log_quoted="$(quote_sh "$REPO_DIR/data/logs/noon-watchdog.log")"
  printf 'cd %s && mkdir -p data/logs && %s scripts/noon_watchdog.py >> %s 2>&1' "$repo_quoted" "$PYTHON_BIN" "$log_quoted"
}
...
build_cron_command() {
  ...
  case "$CRON_MODE" in
    ...
    root-su|ugreen-root)
      ...
      printf '%s - %s -c %s' "$SU_BIN" "$(quote_sh "$RUN_AS_USER")" "$(quote_sh "$watchdog_command")"
      ;;
    *)
...
crontab "$tmp_file"
if [[ "$CRON_MODE" == "root-su" || "$CRON_MODE" == "ugreen-root" ]]; then
  echo "Installed root NAS dashboard noon watchdog cron for: $REPO_DIR as $RUN_AS_USER"
else
  echo "Installed NAS dashboard noon watchdog cron for: $REPO_DIR"
fi
